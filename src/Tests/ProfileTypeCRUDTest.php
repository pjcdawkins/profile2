...
class ProfileTypeCRUDTest extends WebTestBase {
  ...
  function testCRUDUI() {
    ...
    $this->assertRaw(SafeMarkup::format('%label profile type has been created.', ['%label' => $label]));
    ...
    $this->assertRaw(SafeMarkup::format('Edit %label profile type', ['%label' => $label]));
    $edit = [
      'registration' => 1,
    ];
    $this->drupalPostForm(NULL, $edit, t('Save'));
    $this->assertUrl('admin/config/people/profiles/types');
    $this->assertRaw(SafeMarkup::format('%label profile type has been updated.', ['%label' => $label]));
    ...
    $this->assertRaw(SafeMarkup::format('Saved %label configuration.', ['%label' => $field_label]));
